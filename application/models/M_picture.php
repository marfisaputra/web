...
class M_picture extends CI_Model {
    ...
    function tampil_list() {
        $sql = "SELECT id_pic,id_user,nm,pic,ket FROM ".$this->tablename;
    ...
    function get_data($id) { // menampilkan data page
        $sql = "SELECT id_pic,id_user,nm,pic,ket FROM ".$this->tablename. ' WHERE '.$this->key.' = ?';
        $data[$this->key] = $id;
    ...
    function create($data) { // membuat create post picture
    ...
    function update($data, $id) { //mengubah post picture
        $key[$this->key] = $id;
